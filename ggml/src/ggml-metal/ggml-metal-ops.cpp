...
static int ggml_metal_op_encode_impl(ggml_metal_op_t ctx, int idx) {
    ...
    switch (node->op) {
        ...
        case GGML_OP_DSV41_ENGRAM:
            {
                n_fuse = ggml_metal_op_dsv41_engram(ctx, idx);
            } break;
        case GGML_OP_DSV41_ROPE:
            {
                n_fuse = ggml_metal_op_dsv41_rope(ctx, idx);
            } break;
        case GGML_OP_DSV41_HC_SPLIT:
            {
                n_fuse = ggml_metal_op_dsv41_hc_split(ctx, idx);
            } break;
        case GGML_OP_DSV41_SWIGLU:
            {
                n_fuse = ggml_metal_op_dsv41_swiglu(ctx, idx);
            } break;
        case GGML_OP_DSV41_SET_ROWS:
            {
                n_fuse = ggml_metal_op_dsv41_set_rows(ctx, idx);
            } break;
        case GGML_OP_DSV41_INDEX_SCORES:
            {
                n_fuse = ggml_metal_op_dsv41_index_scores(ctx, idx);
            } break;
        case GGML_OP_DSV41_SELECT:
            {
                n_fuse = ggml_metal_op_dsv41_select(ctx, idx);
            } break;
        case GGML_OP_DSV41_ATTN:
            {
                n_fuse = ggml_metal_op_dsv41_attn(ctx, idx);
            } break;
        case GGML_OP_DSV41_POOL:
            {
                n_fuse = ggml_metal_op_dsv41_pool(ctx, idx);
            } break;
...
int ggml_metal_op_unary(ggml_metal_op_t ctx, int idx) {
    ...
    if (ctx->use_fusion()) {
        ...
        if (fusion && fusion->id == GGML_METAL_FUSION_QWEN4EXP_HC_REDUCE) {
            ctx->count_fusions(fusion);
            return ggml_metal_op_qwen4exp_hc_reduce_impl(ctx, idx + 1, op->src[0], true, n);
        }
        if (fusion) {
            fuse_sigmoid_scale = fusion->id == GGML_METAL_FUSION_SIGMOID_SCALE;
            fuse_softplus_sqrt = fusion->id == GGML_METAL_FUSION_SOFTPLUS_SQRT;
            fuse_scale_silu = fusion->id == GGML_METAL_FUSION_SCALE_SILU;
...
static int ggml_metal_op_try_moe_weights(ggml_metal_op_t ctx, int idx) {
    int n = 1;
    const ggml_metal_fusion * fusion = ctx->can_fuse(idx, GGML_METAL_FUSION_FULL, &n);
    if (!fusion || fusion->id != GGML_METAL_FUSION_MOE_WEIGHTS) {
        return 0;
    }
    ctx->count_fusions(fusion);

    const ggml_tensor * get_rows = ctx->node(idx);
    const ggml_tensor * clamp_op = ctx->node(idx + 2);
    const ggml_tensor * scale_op = ctx->node(idx + 4);
    ...
    ggml_metal_kargs_moe_weights args = {
        ...
        /*.nb_d1         =*/ scale_op->nb[1],
        /*.nb_d2         =*/ scale_op->nb[2],
        /*.clamp_min     =*/ ggml_get_op_params_f32(clamp_op, 0),
        /*.clamp_max     =*/ ggml_get_op_params_f32(clamp_op, 1),
        /*.scale         =*/ ggml_get_op_params_f32(scale_op, 0),
        /*.bias          =*/ ggml_get_op_params_f32(scale_op, 1),
    };

    ggml_metal_encoder_t enc = ctx->enc;
    auto pipeline = ggml_metal_library_get_pipeline_moe_weights(ctx->lib);
    ggml_metal_encoder_set_pipeline(enc, pipeline);
    ggml_metal_encoder_set_bytes (enc, &args, sizeof(args), 0);
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(probs),    1);
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(ids),      2);
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(scale_op), 3);
    ggml_metal_encoder_dispatch_threadgroups(enc, args.n_tokens, 1, 1, 32, 1, 1);

    if (ggml_metal_fusion_info_debug(ctx->finfo) > 1) {
        GGML_LOG_DEBUG("%s: fuse: MoE weights\n", __func__);
    }
...
int ggml_metal_op_get_rows(ggml_metal_op_t ctx, int idx) {
    if (ctx->use_fusion()) {
        const int n_fuse = ggml_metal_op_try_moe_weights(ctx, idx);
        if (n_fuse > 0) {
            return n_fuse;
        }
    }
...
int ggml_metal_op_dsv41_engram(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    const ggml_tensor * x = op->src[0];
    const ggml_tensor * key = op->src[1];
    ggml_metal_kargs_dsv41_engram args = {
        /*.dim    =*/ (int32_t) x->ne[0],
        /*.hc     =*/ (int32_t) x->ne[1],
        /*.n_rows =*/ (int32_t) ggml_nrows(x),
        /*.eps    =*/ ggml_get_op_params_f32(op, 0),
        /*.clamp  =*/ ggml_get_op_params_f32(op, 1),
        /*.nb_x1  =*/ x->nb[1],
        /*.nb_x2  =*/ x->nb[2],
        /*.nb_k1  =*/ key->nb[1],
        /*.nb_k2  =*/ key->nb[2],
    ...
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op), 5);
    const int nsg = std::min(4, args.n_rows);
    ggml_metal_encoder_dispatch_threadgroups(enc, (args.n_rows + nsg - 1)/nsg, 1, 1, 32, nsg, 1);
    return 1;
}

int ggml_metal_op_dsv41_set_rows(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    const ggml_tensor * x = op->src[0];
    const auto type = (ggml_dsv41_quant_type) ggml_get_op_params_i32(op, 0);
    ggml_metal_kargs_dsv41_set_rows args = {
        /*.dim        =*/ (int32_t) x->ne[0],
        /*.n_rows     =*/ (int32_t) op->ne[1],
        /*.n_bits     =*/ type == GGML_DSV41_QUANT_MXFP8 ? 8 : 4,
        /*.block_size =*/ type == GGML_DSV41_QUANT_NVFP4 ? 16 : 32,
        /*.nb_x1      =*/ x->nb[1],
        /*.nb_i0      =*/ op->src[1]->nb[0],
        /*.nb_d1      =*/ op->nb[1],
    ...
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op->src[1]), 2);
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op), 3);
    const int nsg = std::min(4, args.dim/32);
    ggml_metal_encoder_dispatch_threadgroups(enc, (args.dim + 32*nsg - 1)/(32*nsg), x->ne[1], 1, 32, nsg, 1);
    return 1;
}
...
int ggml_metal_op_dsv41_index_scores(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    const auto * q = op->src[0], * candidates = op->src[4];
    ...
    auto enc = ctx->enc;
    const bool matrix = ggml_metal_op_dsv41_index_extra_cache(op) > 0 && ggml_metal_device_get_props(ctx->dev)->has_simdgroup_mm;
    auto scratch = ggml_metal_get_buffer_id(op);
    scratch.offs += ggml_nbytes(op);
    if (matrix) {
        auto bounds = ggml_metal_library_get_pipeline_dsv41_index_matrix(ctx->lib, true);
        ggml_metal_encoder_set_pipeline(enc, bounds);
        ggml_metal_encoder_set_bytes(enc, &args, sizeof(args), 0);
        ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(q), 1);
        ggml_metal_encoder_set_buffer(enc, scratch, 2);
        ggml_metal_encoder_dispatch_threadgroups(enc, op->ne[1], 1, 1, 32, 4, 1);
        ggml_metal_op_concurrency_reset(ctx);
    }
    auto pipeline = matrix ? ggml_metal_library_get_pipeline_dsv41_index_matrix(ctx->lib, false) : ggml_metal_library_get_pipeline_base(ctx->lib, op->op);
    ggml_metal_encoder_set_pipeline(enc, pipeline);
    ggml_metal_encoder_set_bytes(enc, &args, sizeof(args), 0);
    for (int i = 0; i < 5; ++i) {
...
int ggml_metal_op_dsv41_select(ggml_metal_op_t ctx, int idx) {
    ...
    ggml_metal_kargs_dsv41_select args = {
        ...
        /*.ratio            =*/ ggml_get_op_params_i32(op, 1),
...
static bool ggml_metal_dsv41_attn_unpack_selected(const ggml_tensor * op) {
    return op->src[0]->ne[2] == 1 && !getenv("GGML_METAL_DSV41_ATTN_UNPACK_ALL");
}

static bool ggml_metal_dsv41_attn_bf16_query(const ggml_tensor * q) {
    while (q->view_src) { q = q->view_src; }
    return q->op == GGML_OP_DSV41_ROPE || (q->op == GGML_OP_DSV41_ACT_QUANT && ggml_get_op_params_i32(q, 0) == GGML_DSV41_QUANT_BF16);
}

static bool ggml_metal_dsv41_attn_precompute_scores(const ggml_tensor * op) {
...
size_t ggml_metal_op_dsv41_attn_extra_cache(const ggml_tensor * op) {
    ...
    const size_t kv_rows = op->src[5] ? (selected ? op->src[4]->ne[0] : op->src[5]->ne[1]) : 0;
    const size_t rows = raw_rows + kv_rows;
    const size_t scores = ggml_metal_dsv41_attn_precompute_scores(op) ? rows*op->src[0]->ne[1] : 0;
    const size_t copies = ggml_metal_dsv41_attn_transpose_scores(op) ? 2 : 1;
    return (copies*rows*op->src[0]->ne[0] + scores)*sizeof(float);
}

int ggml_metal_op_dsv41_attn(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    const auto * q = op->src[0], * indices = op->src[4], * kv = op->src[5];
    const int threads = ggml_metal_dsv41_attn_threads(op);
    ggml_metal_kargs_dsv41_attn args = {
        /*.dim    =*/ (int32_t) q->ne[0],
        /*.heads  =*/ (int32_t) q->ne[1],
        /*.tokens =*/ (int32_t) q->ne[2],
        /*.n_ring =*/ (int32_t) op->src[1]->ne[1],
        /*.n_kv   =*/ kv ? (int32_t) kv->ne[1] : 0,
        /*.top_k  =*/ indices ? (int32_t) indices->ne[0] : 0,
        /*.window =*/ ggml_get_op_params_i32(op, 0),
        /*.ratio  =*/ ggml_get_op_params_i32(op, 1),
        /*.window_start =*/ ggml_get_op_params_i32(op, 2),
        /*.selected =*/ ggml_metal_dsv41_attn_unpack_selected(op),
        /*.precomputed =*/ ggml_metal_dsv41_attn_precompute_scores(op),
        /*.transposed =*/ ggml_metal_dsv41_attn_transpose_scores(op),
        /*.slices =*/ ggml_metal_dsv41_attn_precompute_scores(op) && !getenv("GGML_METAL_DSV41_ATTN_SINGLE_SLICE") ? int(q->ne[0]/threads) : 1,
    ...
    if (unpacked) {
        ...
        if (kv) { kv_buffer.offs += size_t(args.selected ? args.window : args.n_ring)*args.dim*sizeof(float); }
        args.nb_r1 = args.nb_k1 = args.dim*sizeof(float);
    }
    const char * matrix_option = getenv("GGML_METAL_DSV41_ATTN_MATRIX_REGISTER");
    const auto * properties = ggml_metal_device_get_props(ctx->dev);
    const bool matrix = (!matrix_option || atoi(matrix_option) > 0) && unpacked && !args.selected && args.tokens >= 64 && args.dim == 512 && args.heads % 8 == 0 && properties->has_bfloat && properties->has_simdgroup_mm && ggml_metal_dsv41_attn_bf16_query(q);
    auto pipeline = matrix ? ggml_metal_library_get_pipeline_dsv41_attn_matrix(ctx->lib) : unpacked ? ggml_metal_library_get_pipeline_dsv41_attn(ctx->lib, false, false) : ggml_metal_library_get_pipeline_base(ctx->lib, op->op);
    ggml_metal_encoder_set_pipeline(enc, pipeline);
    ggml_metal_encoder_set_bytes(enc, &args, sizeof(args), 0);
    for (int i = 0; i < 6; ++i) {
    ...
    ggml_metal_encoder_dispatch_threadgroups(enc, matrix ? args.heads/8 : args.heads, args.tokens, unpacked ? args.slices : 1, matrix ? 128 : unpacked ? threads : 64, 1, 1);
    return 1;
}

int ggml_metal_op_dsv41_pool(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    ggml_metal_kargs_dsv41_pool args = {
        /*.dim    =*/ (int32_t) op->ne[0],
    ...
    ggml_metal_encoder_set_pipeline(enc, pipeline);
    ggml_metal_encoder_set_bytes(enc, &args, sizeof(args), 0);
    for (int i = 0; i < 3; ++i) { ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op->src[i]), i + 1); }
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op), 4);
    ggml_metal_encoder_dispatch_threadgroups(enc, (args.dim + 127)/128, op->ne[1], 1, 128, 1, 1);
    return 1;
}

int ggml_metal_op_dsv41_swiglu(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    ggml_tensor * dst = op;
    int n_fuse = 1;
    if (ctx->use_fusion()) {
        int n = 1;
        const auto * fusion = ctx->can_fuse(idx, GGML_METAL_FUSION_FULL, &n);
        if (fusion && fusion->id == GGML_METAL_FUSION_DSV41_SWIGLU_MXFP8) {
            n_fuse = n;
            dst = ctx->node(idx + n - 1);
            ctx->count_fusions(fusion);
        }
    ...
    ggml_metal_kargs_dsv41_swiglu args = {
...
int ggml_metal_op_dsv41_rope(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    const ggml_tensor * x = op->src[0];
    ggml_metal_kargs_dsv41_rope args = {
        /*.dim    =*/ (int32_t) x->ne[0],
        /*.heads  =*/ (int32_t) x->ne[1],
        /*.n_rows =*/ (int32_t) ggml_nrows(x),
        /*.offset =*/ (int32_t) (x->ne[0] - op->src[1]->ne[0]),
        /*.sign   =*/ ggml_get_op_params_i32(op, 0) ? -1.0f : 1.0f,
        /*.nb_x1  =*/ x->nb[1],
        /*.nb_x2  =*/ x->nb[2],
        /*.nb_r1  =*/ op->src[1]->nb[1],
    };
    ...
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op->src[1]), 2);
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op), 3);
    ggml_metal_encoder_dispatch_threadgroups(enc, (args.dim/2 + 127)/128, args.n_rows, 1, 128, 1, 1);
    return 1;
}

int ggml_metal_op_dsv41_hc_split(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->node(idx);
    ggml_metal_kargs_dsv41_hc_split args = {
...
int ggml_metal_op_dsv4_hc(ggml_metal_op_t ctx, int idx) {
    ...
    switch (op->op) {
        case GGML_OP_DSV4_HC_COMB:
            {
                auto pipeline = ggml_metal_library_get_pipeline_dsv4_hc(ctx->lib, op->op, false);
                ggml_metal_encoder_set_pipeline(enc, pipeline);
        ...
        case GGML_OP_DSV4_HC_PRE:
            {
                ...
                if (fusion && (fusion->id == GGML_METAL_FUSION_DSV4_HC_PRE_NORM || fusion->id == GGML_METAL_FUSION_DSV41_HC_PRE_NORM)) {
                    const bool bf16 = fusion->id == GGML_METAL_FUSION_DSV41_HC_PRE_NORM;
                    const ggml_tensor * norm = ctx->node(idx + (bf16 ? 2 : 1));
                    const ggml_tensor * mul = ctx->node(idx + (bf16 ? 3 : 2));
                    ...
                    auto pipeline = ggml_metal_library_get_pipeline_dsv4_hc_pre_norm(ctx->lib, bf16);
                    const int nth = bf16 ? 1024 : args.n_embd/4;
        ...
        case GGML_OP_DSV4_HC_POST:
            {
                auto pipeline = ggml_metal_library_get_pipeline_dsv4_hc(ctx->lib, op->op, ggml_get_op_params_i32(op, 0));
                ggml_metal_encoder_set_pipeline(enc, pipeline);
...
struct ggml_metal_moe_combine_fusion {
...
static int ggml_metal_op_moe_combine_fused(
        ggml_metal_op_t ctx,
        int idx,
        const ggml_metal_moe_combine_fusion & fusion) {
    for (int i = 1; i < fusion.n_fuse; ++i) {
        if (!ggml_metal_op_concurrency_check(ctx, ctx->node(idx + i))) {
            ggml_metal_op_concurrency_reset(ctx);
            break;
        }
    ...
    ggml_metal_kargs_moe_combine args = {
        /*.n_embd   =*/ (int32_t) fusion.experts->ne[0],
        /*.n_expert =*/ (int32_t) fusion.experts->ne[1],
        /*.n_tokens =*/ (int32_t) fusion.experts->ne[2],
    };

    ggml_metal_encoder_t enc = ctx->enc;
    auto pipeline = ggml_metal_library_get_pipeline_moe_combine(ctx->lib);
    ggml_metal_encoder_set_pipeline(enc, pipeline);
    ggml_metal_encoder_set_bytes (enc, &args, sizeof(args), 0);
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(fusion.experts), 1);
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(fusion.weights), 2);
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(fusion.dst),     3);

    ggml_metal_encoder_dispatch_threadgroups(enc, (args.n_embd + 31)/32, args.n_tokens, 1, 32, 1, 1);
...
static int ggml_metal_op_try_dsv4_hc_post_add(ggml_metal_op_t ctx, int idx) {
    ...
    ggml_metal_encoder_t enc = ctx->enc;
    auto pipeline = ggml_metal_library_get_pipeline_dsv4_hc_post_add(ctx->lib, ggml_get_op_params_i32(hc_post, 0));
    ggml_metal_encoder_set_pipeline(enc, pipeline);
    ggml_metal_encoder_set_bytes (enc, &args, sizeof(args), 0);
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(x),        1);
...
int ggml_metal_op_bin(ggml_metal_op_t ctx, int idx) {
    ...
    if (ctx->use_fusion()) {
        ...
        if (fusion && fusion->id == GGML_METAL_FUSION_DSV4_HC_AFFINE) {
            ctx->count_fusions(fusion);
            return ggml_metal_op_try_dsv4_hc_affine(ctx, idx);
        }
        if (fusion && fusion->id == GGML_METAL_FUSION_DSV4_HC_POST_ADD) {
            ctx->count_fusions(fusion);
            return ggml_metal_op_try_dsv4_hc_post_add(ctx, idx);
        }

        if (fusion && fusion->id == GGML_METAL_FUSION_MOE_COMBINE) {
            const ggml_tensor * mul = ctx->node(idx);
            ggml_metal_moe_combine_fusion args = { mul->src[0], mul->src[1], ctx->node(idx + n - 1), n };
            ctx->count_fusions(fusion);
            return ggml_metal_op_moe_combine_fused(ctx, idx, args);
        }
...
int ggml_metal_op_norm(ggml_metal_op_t ctx, int idx) {
    ...
    if (fusion && (fusion->id == GGML_METAL_FUSION_RMS_NORM_ROPE || fusion->id == GGML_METAL_FUSION_RMS_NORM_ROPE_CPY)) {
    ...
    if (fusion && fusion->id == GGML_METAL_FUSION_NORM_SCALE) {
        args.post_scale = ggml_get_op_params_f32(ctx->node(idx + 1), 0);
        n_fuse = n;
        fuse_scale = true;
        ctx->count_fusions(fusion);
    }
    ...
    auto pipeline = ggml_metal_library_get_pipeline_norm(lib, op, fuse_scale ? 1 : n_fuse);
