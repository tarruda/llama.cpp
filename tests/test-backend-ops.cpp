...
struct test_moe_combine_fuse : public test_case {
    const int64_t n_embd;
    const int64_t n_expert;
    const int64_t n_tokens;
    const bool reuse_view;

    test_moe_combine_fuse(int64_t n_embd, int64_t n_expert, int64_t n_tokens, bool reuse_view = false)
        : n_embd(n_embd), n_expert(n_expert), n_tokens(n_tokens), reuse_view(reuse_view) {}

    std::string op_desc(ggml_tensor * t) override {
        GGML_UNUSED(t);
        return "MOE_COMBINE_FUSE";
    }

    std::string vars() override {
        return VARS_TO_STR4(n_embd, n_expert, n_tokens, reuse_view);
    }

    bool run_whole_graph() override { return true; }

    ggml_tensor * build_graph(ggml_context * ctx) override {
        ggml_tensor * experts = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, n_embd, n_expert, n_tokens);
        ggml_tensor * weights = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, 1, n_expert, n_tokens);
        ggml_tensor * weighted = ggml_mul(ctx, experts, weights);
        ggml_build_forward_expand(gf, weighted);

        std::vector<ggml_tensor *> views(n_expert);
        for (int64_t i = 0; i < n_expert; ++i) {
            views[i] = ggml_view_2d(ctx, weighted, n_embd, n_tokens, weighted->nb[2], i*weighted->nb[1]);
...
struct test_moe_weights_fuse : public test_case {
    const int64_t n_expert;
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        weights = ggml_scale_bias(ctx, weights, scale, bias);
        ggml_set_name(weights, "out");
        return weights;
    }
            ...
            if (t->type == GGML_TYPE_I32) {
                ...
                for (int64_t it = 0; it < n_tokens; ++it) {
                    for (int64_t ie = 0; ie < n_expert_used; ++ie) {
                        data[it*n_expert_used + ie] = (int32_t) ((37*ie + 11*it) % n_expert);
                    }
                }
                ggml_backend_tensor_set(t, data.data(), 0, data.size()*sizeof(int32_t));
            } else if (!ggml_is_view_op(t->op)) {
                init_tensor_uniform(t, 0.01f, 2.0f);
            }
        }
    }
};
...
struct test_dsv41_set_rows : public test_case {
    const ggml_dsv41_quant_type type;
    const int64_t tokens;
    const bool strided;

    test_dsv41_set_rows(ggml_dsv41_quant_type type, int64_t tokens, bool strided) : type(type), tokens(tokens), strided(strided) {}
    std::string vars() override { return VARS_TO_STR3(type, tokens, strided); }
    double max_nmse_err() override { return 0; }

    ggml_tensor * build_graph(ggml_context * ctx) override {
        const int64_t dim = type == GGML_DSV41_QUANT_MXFP4 ? 128 : 512;
        const int64_t bytes = type == GGML_DSV41_QUANT_MXFP8 ? dim/32*33 : ggml_row_size(type == GGML_DSV41_QUANT_MXFP4 ? GGML_TYPE_MXFP4 : GGML_TYPE_NVFP4, dim);
        auto * cache = ggml_new_tensor_2d(ctx, GGML_TYPE_I8, strided ? bytes + 11 : bytes, tokens + 7);
        auto * x = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, strided ? dim + 32 : dim, tokens);
        auto * ids = ggml_new_tensor_2d(ctx, GGML_TYPE_I32, strided ? 2 : 1, tokens);
        if (strided) {
            cache = ggml_view_2d(ctx, cache, bytes, tokens + 7, cache->nb[1], 5);
            x = ggml_view_2d(ctx, x, dim, tokens, x->nb[1], 32*sizeof(float));
            ids = ggml_view_2d(ctx, ids, 1, tokens, ids->nb[1], sizeof(int32_t));
        }
        ids = ggml_transpose(ctx, ids);
        ggml_set_name(ids, "dsv41_rows");
        return ggml_dsv41_set_rows(ctx, cache, x, ids, type);
    }

    void initialize_tensors(ggml_context * ctx) override {
        test_case::initialize_tensors(ctx);
        auto * ids = ggml_get_tensor(ctx, "dsv41_rows");
        for (int64_t i = 0; i < tokens; ++i) {
            const int32_t id = tokens > 1 && i % 2 == 0 ? -1 : (7*i + 3) % (tokens + 7);
            ggml_backend_tensor_set(ids, &id, i*ids->nb[0], sizeof(id));
        }
    }
};
...
struct test_dsv41_index_scores : public test_case {
    ...
    test_dsv41_index_scores(int64_t dim, int64_t tokens, bool masked, bool strided, int exponent_shift = 0) : dim(dim), tokens(tokens), masked(masked), strided(strided), exponent_shift(exponent_shift) {}
    std::string vars() override { return VARS_TO_STR5(dim, tokens, masked, strided, exponent_shift); }
    double max_nmse_err() override { return 0; }

    ggml_tensor * build_graph(ggml_context * ctx) override {
        const int64_t heads = dim == 128 ? 32 : 4;
        const int64_t n_keys = tokens == 1 ? 1024 : 33;
        auto * query = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, dim, strided ? 2*heads : heads, tokens);
        ggml_set_name(query, "dsv41_index_query_input");
        auto * q = ggml_dsv41_act_quant(ctx, query, GGML_DSV41_QUANT_MXFP4);
        ...
        auto * key_values = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, dim, n_keys);
        ggml_set_name(key_values, "dsv41_index_key_input");
        keys = ggml_dsv41_set_rows(ctx, strided ? ggml_view_2d(ctx, keys, bytes, n_keys, keys->nb[1], 5) : keys,
                key_values, ggml_cast(ctx, key_ids, GGML_TYPE_I32), GGML_DSV41_QUANT_MXFP4);
    ...
    void initialize_tensors(ggml_context * ctx) override {
        test_case::initialize_tensors(ctx);
        if (exponent_shift) {
            for (const char * name : { "dsv41_index_query_input", "dsv41_index_key_input" }) {
                auto * tensor = ggml_get_tensor(ctx, name);
                std::vector<float> values(ggml_nelements(tensor));
                ggml_backend_tensor_get(tensor, values.data(), 0, ggml_nbytes(tensor));
                for (float & value : values) { value = std::ldexp(value, exponent_shift); }
...
struct test_dsv41_engram : public test_case {
    ...
    test_dsv41_engram(int64_t dim, int64_t tokens, bool strided) : dim(dim), tokens(tokens), strided(strided) {}

    std::string vars() override { return VARS_TO_STR3(dim, tokens, strided); }
    double max_nmse_err() override { return 1e-11; }

    ggml_tensor * build_graph(ggml_context * ctx) override {
        auto * x = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, strided ? 2*dim : dim, 4, tokens);
        ggml_set_name(x, "engram_x");
        if (strided) { x = ggml_view_3d(ctx, x, dim, 4, tokens, x->nb[1], x->nb[2], dim*sizeof(float)); }
        auto * kv = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, 5*dim, tokens);
        auto * key = ggml_view_3d(ctx, kv, dim, 4, tokens, dim*sizeof(float), kv->nb[1], 0);
        auto * value = ggml_view_2d(ctx, kv, dim, tokens, kv->nb[1], 4*dim*sizeof(float));
        auto * weight = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, dim, 4);
        return ggml_dsv41_engram(ctx, x, key, value, weight, 1e-20f, 1e-6f);
    }

    void initialize_tensors(ggml_context * ctx) override {
        test_case::initialize_tensors(ctx);
        auto * x = ggml_get_tensor(ctx, "engram_x");
        std::vector<float> zeros(x->ne[0], 0.0f);
        ggml_backend_tensor_set(x, zeros.data(), 0, zeros.size()*sizeof(float));
    }
};

struct test_dsv41_rope : public test_case {
    const int64_t dim;
    const int64_t heads;
    const int64_t tokens;
    const bool strided;
    ...
    test_dsv41_rope(int64_t dim, int64_t heads, int64_t tokens, bool strided, bool inverse) :
        dim(dim), heads(heads), tokens(tokens), strided(strided), inverse(inverse) {}

    std::string vars() override { return VARS_TO_STR5(dim, heads, tokens, strided, inverse); }
    double max_nmse_err() override { return 0; }

    ggml_tensor * build_graph(ggml_context * ctx) override {
        auto * x = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, strided ? 2*dim : dim, heads, tokens);
        if (strided) { x = ggml_view_3d(ctx, x, dim, heads, tokens, x->nb[1], x->nb[2], dim*sizeof(float)); }
        auto * cs = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, 64, tokens);
        return ggml_dsv41_rope(ctx, x, cs, inverse);
    }
};

struct test_dsv41_hc_split : public test_case {
    const int64_t tokens;
...
struct test_dsv41_swiglu : public test_case {
    const float limit;
    const bool weighted;
    const bool strided;
    const bool mxfp8;
...
struct test_dsv4_hc_pre_norm : public test_dsv4_hc {
    ...
    std::string op_desc(ggml_tensor * t) override {
        GGML_UNUSED(t);
        return bf16 ? "DSV41_HC_PRE_NORM" : "DSV4_HC_PRE_NORM";
    }
    ...
    std::string vars() override {
        return VARS_TO_STR4(n_embd, n_tokens, eps, bf16);
    }

    test_dsv4_hc_pre_norm(int64_t n_embd = 4096, int64_t n_tokens = 17, float eps = 1e-6f, bool bf16 = false)
        : n_embd(n_embd), n_tokens(n_tokens), eps(eps), bf16(bf16) {}
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        out = ggml_dsv4_hc_pre(ctx, x, weights);
        if (bf16) { out = ggml_dsv41_act_quant(ctx, out, GGML_DSV41_QUANT_BF16); }
...
struct test_mul_mat_f32_inputs : public test_mul_mat {
    ...
    test_mul_mat_f32_inputs(int64_t m, int64_t n, int64_t k, std::array<int64_t, 2> bs = {1, 1}, std::array<int64_t, 2> nr = {1, 1}, ggml_type type_a = GGML_TYPE_F32, float input_scale = 1.0f) :
    ...
    double max_nmse_err() override { return ggml_is_quantized(type_a) ? test_mul_mat::max_nmse_err() : 1e-11; }
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_eval() {
    ...
    test_cases.emplace_back(new test_moe_combine_fuse(1,    2,  1));
    test_cases.emplace_back(new test_moe_combine_fuse(31,   10, 1));
    test_cases.emplace_back(new test_moe_combine_fuse(33,   15, 3));
    test_cases.emplace_back(new test_moe_combine_fuse(2560, 10, 1));
    test_cases.emplace_back(new test_moe_combine_fuse(33,   16, 2));
    test_cases.emplace_back(new test_moe_combine_fuse(33,   10, 2, true));
    ...
    for (auto type : { GGML_DSV41_QUANT_MXFP8, GGML_DSV41_QUANT_MXFP4, GGML_DSV41_QUANT_NVFP4 }) {
        for (bool strided : { false, true }) {
            test_cases.emplace_back(new test_dsv41_set_rows(type, 1, strided));
            test_cases.emplace_back(new test_dsv41_set_rows(type, 33, strided));
        }
    }
    for (int64_t dim : { 63, 64, 5120 }) {
        for (bool strided : { false, true }) {
    ...
    for (int shift : { -40, 40 }) {
        test_cases.emplace_back(new test_dsv41_index_scores(128, 7, true, true, shift));
    }
    ...
    for (int64_t dim : { 64, 128, 512 }) {
        for (bool inverse : { false, true }) {
            ...
            test_cases.emplace_back(new test_dsv41_rope(dim, 4, 33, true, inverse));
        }
    }
    for (int iterations : { 1, 20 }) {
        for (bool strided : { false, true }) {
            test_cases.emplace_back(new test_dsv41_hc_split(1, iterations, strided));
    ...
    test_cases.emplace_back(new test_dsv4_hc_pre_norm(5120, 1, 1e-6f, true));
    test_cases.emplace_back(new test_dsv4_hc_pre_norm(5120, 128, 1e-6f, true));
    ...
    test_cases.emplace_back(new test_dsv4_hc_post(1, 1, false, true));
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    for (int64_t m : {63, 64, 65}) {
        for (int64_t k : {256, 4096}) {
            test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_IQ3_XXS, GGML_TYPE_F32, 128, 1, true, m, 256, k, true));
        }
    }
