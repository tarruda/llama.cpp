...
llama_memory_dsv41::llama_memory_dsv41(const llama_model & model, uint32_t n_ctx, uint32_t n_seq_max, uint32_t n_ubatch) :
    ...
    const ggml_init_params params = { ggml_tensor_overhead()*hparams.n_layer()*5, nullptr, true };
    ctx.reset(ggml_init(params));
    if (!ctx) {
        throw std::runtime_error("failed to create DeepSeek-V4.1 state context");
    }
    layers.resize(hparams.n_layer());
    const auto create = [&](ggml_type type, int64_t width, int64_t rows, const char * name, int il) {
        auto * tensor = ggml_new_tensor_2d(ctx.get(), type, width, rows);
    ...
    for (uint32_t il = 0; il < hparams.n_layer(); ++il) {
        ...
        layer.raw = create(GGML_TYPE_I8, dim/32*33, n_ring, "raw", il);
        ...
        layer.kv = create(GGML_TYPE_I8, dim/64*sizeof(block_nvfp4), rows, "kv", il);
        layer.index = create(GGML_TYPE_I8, hparams.indexer_head_size/32*sizeof(block_mxfp4), rows, "index", il);
        global_bytes += ggml_nbytes(layer.kv) + ggml_nbytes(layer.index);
        if (ratio > 1) {
            layer.comp_kv = create(GGML_TYPE_F32, dim, n_ring, "comp_kv", il);
            layer.comp_score = create(GGML_TYPE_F32, dim, n_ring, "comp_score", il);
    ...
    if (!buffer) {
        throw std::runtime_error("failed to allocate DeepSeek-V4.1 state");
    }
    clear(true);
...
llama_pos llama_memory_dsv41::seq_pos_min(llama_seq_id seq_id) const {
    return seq_id == 0 && !tokens.empty() ? 0 : -1;
}
...
std::map<ggml_backend_buffer_type_t, size_t> llama_memory_dsv41::memory_breakdown() const {
    ...
    const size_t bytes = hparams.no_alloc ? ggml_backend_alloc_ctx_tensors_from_buft_size(ctx.get(), buft) : ggml_backend_buffer_get_size(buffer.get());
    return {{buft, bytes + tokens.capacity()*sizeof(llama_token)}};
}

static void * dsv41_row(ggml_tensor * tensor, llama_pos pos) {
    GGML_ASSERT(tensor && pos >= 0 && pos < tensor->ne[1]);
    return (char *) tensor->data + pos*tensor->nb[1];
}

void llama_memory_dsv41::write_raw(int il, llama_pos pos, const float * values) {
    quantize_row_mxfp8_act_ref(values, (uint8_t *) dsv41_row(layers[il].raw, pos%n_ring), hparams.n_embd_head_k(il));
}

void llama_memory_dsv41::read_raw(int il, llama_pos pos, float * values) const {
    dequantize_row_mxfp8_act((uint8_t *) dsv41_row(layers[il].raw, pos%n_ring), values, hparams.n_embd_head_k(il));
}

void llama_memory_dsv41::write_kv(int il, llama_pos pos, const float * values) {
    quantize_row_nvfp4_act_ref(values, (block_nvfp4 *) dsv41_row(layers[il].kv, pos), hparams.n_embd_head_k(il));
}

void llama_memory_dsv41::read_kv(int il, llama_pos pos, float * values) const {
    const auto source = hparams.dsv41_kv_source[il];
    dequantize_row_nvfp4((block_nvfp4 *) dsv41_row(layers[source].kv, pos), values, hparams.n_embd_head_k(il));
}

void llama_memory_dsv41::write_index(int il, llama_pos pos, const float * values) {
    quantize_row_mxfp4_act_ref(values, (block_mxfp4 *) dsv41_row(layers[il].index, pos), hparams.indexer_head_size);
}

void llama_memory_dsv41::read_index(int il, llama_pos pos, float * values) const {
    const auto source = hparams.dsv41_kv_source[il];
    dequantize_row_mxfp4((block_mxfp4 *) dsv41_row(layers[source].index, pos), values, hparams.indexer_head_size);
}

void llama_memory_dsv41::state_write(llama_io_write_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) const {
    if (flags & ~LLAMA_STATE_SEQ_FLAGS_ON_DEVICE) {
        throw std::runtime_error("DeepSeek-V4.1 partial state snapshots are not implemented yet");
    }
    GGML_ASSERT(seq_id == -1 || seq_id == 0);
    const uint32_t header[] = { 0xd5410001, n_ring, (uint32_t) layers.size(), hparams.n_embd_head_k(), hparams.indexer_head_size, (uint32_t) tokens.size(), ring_end };
...
void llama_memory_dsv41::state_read(llama_io_read_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) {
    ...
    try {
        ...
        if (header[0] != 0xd5410001 || header[1] != n_ring || header[2] != layers.size() || header[3] != hparams.n_embd_head_k() || header[4] != hparams.indexer_head_size || header[5] > header[6] || header[6] > n_ctx) {
