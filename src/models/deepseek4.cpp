...
static ggml_tensor * dsv4_build_kq_zero_bias(
        ...
        ggml_tensor * kq_mask,
        int64_t n_head) {
    if (!cparams.kv_unified || !cparams.flash_attn || kq_mask->ne[3] == 1 || kq_mask->ne[1] != 1) {
        return nullptr;
    }
...
ggml_tensor * llama_model_deepseek4::graph::build_hca_compressed_kv_from_state(
    ...
    ggml_tensor * comp = ggml_dsv4_compress(ctx0, kv_state, score_state, state_read_idxs, DSV4_HCA_RATIO, false);
    cb(comp, name, il);
...
ggml_tensor * llama_model_deepseek4::graph::build_overlap_compressed_kv_from_state(
    ...
    ggml_tensor * comp = ggml_dsv4_compress(ctx0, kv_state, score_state, state_read_idxs, (int32_t) ratio, true);
    cb(comp, name, il);
