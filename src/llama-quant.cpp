...
static void llama_tensor_dequantize_impl(
    ...
    const size_t element_offset, const size_t nelements, const int nthread
) {
    ...
    const size_t block_size = tensor->type == GGML_TYPE_F16 || tensor->type == GGML_TYPE_BF16 ? 1 : (size_t) ggml_blck_size(tensor->type);
    const size_t block_size_bytes = ggml_type_size(tensor->type);

    GGML_ASSERT(element_offset % block_size == 0);
    GGML_ASSERT(element_offset + nelements <= (size_t) ggml_nelements(tensor));

    const uint8_t * input = (const uint8_t *) tensor->data + element_offset / block_size * block_size_bytes;

    if (nthread < 2) {
        if (tensor->type == GGML_TYPE_F16) {
            ggml_fp16_to_fp32_row((const ggml_fp16_t *) input, f32_output, nelements);
        } else if (tensor->type == GGML_TYPE_BF16) {
            ggml_bf16_to_fp32_row((const ggml_bf16_t *) input, f32_output, nelements);
        } else if (ggml_is_quantized(tensor->type)) {
            qtype->to_float(input, f32_output, nelements);
        } else {
    ...
    for (int tnum = 0; tnum < nthread; tnum++) {
        ...
        workers.emplace_back(compute, tensor->type, input + in_buff_offs, f32_output + out_buff_offs, thr_elems);
...
static void llama_model_quantize_impl(const std::string & fname_inp, const std::string & fname_out, const llama_model_quantize_params * params) {
    ...
    llama_load_mode load_mode = params->load_mode;
    ...
    for (size_t i = 0; i < tensors.size(); ++i) {
        ...
        if (params->dry_run) {
            ...
            if (!quantize) {
                ...
                const size_t row_size = ggml_row_size(new_type, n_per_row);
                const size_t new_tensor_size = row_size * nelements / n_per_row;

                if (work.size() < new_tensor_size) {
                    work.resize(new_tensor_size);
                }
                new_data = work.data();
                ...
                for (int64_t i03 = 0; i03 < tensor->ne[2]; ++i03) {
                    ...
                    if (tensor->type == GGML_TYPE_F32) {
                        const float * f32_data_03 = (const float *) tensor->data + i03 * nelements_matrix;
                        new_size += llama_tensor_quantize_impl(new_type, f32_data_03, new_data_03, chunk_size, nrows, n_per_row, imatrix_03, workers, nthread_use);
                        continue;
                    }

                    static const int64_t max_f32_chunk_size = 64 * 1024 * 1024;
                    const int64_t nrows_per_f32_chunk = std::max<int64_t>(1, max_f32_chunk_size / n_per_row);

                    for (int64_t first_row = 0; first_row < nrows; first_row += nrows_per_f32_chunk) {
                        const int64_t this_nrows = std::min(nrows - first_row, nrows_per_f32_chunk);
                        const size_t element_offset = i03 * nelements_matrix + first_row * n_per_row;
                        const size_t this_nelements = this_nrows * n_per_row;
                        void * new_data_chunk = (char *) new_data_03 + row_size * first_row;

                        llama_tensor_dequantize_impl(tensor, f32_conv_buf, workers, element_offset, this_nelements, nthread);
                        new_size += llama_tensor_quantize_impl(new_type, (const float *) f32_conv_buf.data(), new_data_chunk, chunk_size, this_nrows, n_per_row, imatrix_03, workers, nthread_use);
                    }
                }
