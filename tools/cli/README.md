...
| `-smoe, --stream-moe` | stream routed experts; forces load-mode none and lazy-mode on (DeepSeek V4.1)<br/>(env: LLAMA_ARG_STREAM_MOE) |
| `--moe-cache MiB` | adaptive routed expert cache in MiB; requires --stream-moe (default: 0 = active-expert working set only)<br/>(env: LLAMA_ARG_MOE_CACHE) |
