...
| `--prefill-mode full\|ced` | prompt processing mode: full = all layers, ced = context encoder/decoder (default: full). CED currently requires one text-generation slot without speculative decoding; models without CED use full processing.<br/>(env: LLAMA_ARG_PREFILL_MODE) |
