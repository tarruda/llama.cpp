...
For DeepSeek V4.1, calibration uses full causal evaluation so that both halves receive importance data for every calibration token. When the model must be streamed, `--stream-moe` forces `--load-mode none --lazy-mode on` and uses one calibration sequence. `--moe-cache` sets the adaptive routed expert cache in MiB; leave memory for other weights, activations, output buffers, context state, and lazy row caches. Only complete corpus chunks are calibrated, and GGUF checkpoints are replaced atomically after a successful write.
