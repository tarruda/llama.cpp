...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            if (value == "auto") {
                ...
                throw std::invalid_argument("invalid prefill mode: " + value);
            }
        }
    ).set_env("LLAMA_ARG_PREFILL_MODE").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--keep"}, "N",
    ...
    add_opt(common_arg(
        {"-smoe", "--stream-moe"},
        "stream routed experts; forces load-mode none and lazy-mode on (DeepSeek V4.1)",
        [](common_params & params) { params.stream_moe = true; }
    ).set_env("LLAMA_ARG_STREAM_MOE"));
    add_opt(common_arg(
        {"--moe-cache"}, "MiB",
        "adaptive routed expert cache in MiB; requires --stream-moe (default: 0 = active-expert working set only)",
        [](common_params & params, int value) {
